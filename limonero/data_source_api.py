...
def is_logged_user_owner_or_admin(data_source):
    return (
        int(data_source.user_id) == int(flask_g.user.id)
...
class DataSourceListApi(Resource):
    ...
    def get():
        ...
        try:
            ...
            for f, transform in list(possible_filters.items()):
                data_sources = apply_filter(
                    data_sources, request.args, f, transform, lambda field: field
                )
            ...
            formats = [
                f
                for f in request.args.get("formats", "").split(",")
                if f in DataSourceFormat.values()
            ]
            if formats:
                data_sources = data_sources.filter(
                    DataSource.format.in_(formats)
                )
            ...
            if request.args.get("list") is None:
                if page is not None and page.isdigit():
                    ...
                    result = {
                        "data": DataSourceListResponseSchema(
                            many=True, only=only, exclude=("permissions",)
                ...
                only = ("id", "name", "tags")
                result = DataSourceListResponseSchema(many=True, only=only).dump(
                    data_sources
                )
            db.session.commit()
            result_code = 200
    ...
    def post():
        ...
        if request.json is not None:
            ...
            json_data["user_login"] = json_data.get(
                "user_login", flask_g.user.login
            )
            json_data["user_name"] = json_data.get(
                "user_name", flask_g.user.name
            )
...
class DataSourceDetailApi(Resource):
    ...
    def get(data_source_id):
        ...
        data_sources = DataSource.query
        data_source = _filter_by_permissions(
            data_sources, list(PermissionType.values())
        )
        ...
        if data_source is not None:
            if names_only:
                attributes = {
                    "attributes": [
                        {"name": attr.name} for attr in data_source.attributes
                    ]
                }
                return attributes
            else:
                ...
                try:
                    ...
                    result, result_code = (
                        dict(
                            status="OK",
                            message=gettext(
                                "%(what)s was successfuly deleted",
                        ...
                        result, result_code = (
                            dict(
                                ...
                                message=gettext(
                                    ...
                                    "have 2 attributes (id and description)"
                                ),
                            ),
                            401,
                        )
        ...
        if request.json is not None:
            ...
            if not error:
                try:
                    ...
                    if data_source is not None:
                        conditions = [
                            DataSourcePermission.data_source_id
                            == data_source_id,
                        ...
                        db.session.commit()
                        result, result_code = (
                            {"message": action_performed, "status": "OK"},
                            200,
                        )
                    else:
                        result, result_code = (
...
class DataSourceUploadApi(Resource):
    ...
    def get():
        ...
        try:
            ...
            identifier, filename, chunk_number = DataSourceUploadApi._get_ids(
                request
            )
            ...
            if not all([storage_id, identifier, filename, chunk_number]):
                ...
                if parsed.scheme == "file":
                    ...
                    if not chunk_path.exists():
                        result, result_code = (
                            {"status": "OK", "message": gettext("Not found")},
                            404,
                        )
                    ...
                    if parsed.port:
                        hdfs = fs.HadoopFileSystem(
                            str_uri,
                            user=extra_params.user or "hadoop",
                    ...
                    chunk_filename = (
                        f"{tmp_path}/{filename}.part{chunk_number:09d}"
                    )
            ...
            log.exception(e)
            return {"status": "ERROR", "message": gettext("Internal error")}, 400
    ...
    def post():
        try:
            ...
            if not all([identifier, filename, chunk_number]):
                ...
                storage = Storage.query.get(storage_id)
                storage_url = (
                    storage.url if storage.url[-1] != "/" else storage.url[:-1]
                )
                ...
                if parsed.scheme == "hdfs":
                    if parsed.port:
                        hdfs = fs.HadoopFileSystem(
                            str_uri, user=hadoop_user, port=int(parsed.port)
                        )
                    else:
                        hdfs = fs.HadoopFileSystem(
                            str_uri,
                ...
                if chunk_number == total_chunks:
                    ...
                    if parsed_path:
                        target_path = f'/{parsed_path.strip("/")}/limonero/data/{final_filename}'
                    else:
                    ...
                    if parsed.scheme == "hdfs":
                        if hu.exists(hdfs, target_path):
                            ...
                            result_code = 500
                        hu.copy_merge(
                            hdfs, tmp_path, target_path, filename, total_chunks
                        )
                    elif parsed.scheme == "file":
                        ...
                        for i in range(total_chunks):
                            number = str(i + 1).rjust(9, "0")
                            name = (
                                f'{tmp_path.rstrip("/")}/{filename}.part{number}'
                            )
                            try:
                                os.remove(name)
                            except:
    ...
    def _try_infer_schema(ds, delim):
        options = {"use_header": True, "delimiter": delim}
...
class DataSourceDownload(MethodView):
    ...
    def get(data_source_id):
        ...
        try:
            ...
            if token is None:
                return json.dumps(
                    {"status": "ERROR", "message": gettext("Missing token")}
                ), 401
            ...
            return json.dumps(
                {
                    "status": "ERROR",
                    "message": gettext(
                        "Invalid or expired token. Refresh the listing page."
                    ),
                }
            ), 500
        if download_token["id"] != data_source_id:
            return json.dumps(
                {"status": "ERROR", "message": gettext("Invalid data source.")}
            ), 401
        ...
        if parsed.scheme == "file":
            name = "{}.{}".format(
                data_source.name.replace(" ", "-"), data_source.format.lower()
            )
            ...
            if data_source.format == "PARQUET" and convert_to_csv:
                ...
                name = name + ".csv"
                result = Response(stream_with_context(do_download(parsed.path)))
            else:
...
class DataSourceInferSchemaApi(Resource):
    ...
    def infer_schema(ds, options):
        ...
        if ds.format in (DataSourceFormat.JDBC,):
            ...
            if parsed.scheme == "hdfs":
                ...
                raise ValueError(
                    gettext("Unsupported filesystem: {fs}", fs=parsed.schema)
                )
            ...
            if parsed.scheme == "hdfs":
                ...
                raise ValueError(
                    gettext("Unsupported filesystem: ") + parsed.scheme
                )
            if ds.format == DataSourceFormat.CSV:
                try:
                    use_header = (
                        options.get("use_header", False)
                        or ds.is_first_line_header
                    )
                    ...
                    if parsed.scheme == "file":
                        ...
                        if is_gzip:
                            ...
                            buffered_reader = codecs.open(
                                parsed.path, "rb", encoding=encoding
                            )
                    elif parsed.scheme == "hdfs":
                    ...
                    quote_char = (
                        quote_char.encode(encoding) if quote_char else None
                    )
                ...
                if ds.url.endswith(".zip"):
                    ...
                    if dbf_name is None:
                        raise ValueError(
                            gettext(
                                "Cannot infer the schema for shapefile "
            ...
            raise ValueError(
                gettext(
                    "Cannot infer the schema for format %(format)s",
                    format=ds.format,
                )
            )
    ...
    def infer_from_hive(ds: DataSource, sql: str) -> None:
        ...
        with hive.connect(
            ...
            with conn.cursor() as cursor:
                ...
                if found:
                    cursor.execute(
                        f"DESCRIBE FORMATTED {found.group(1)}", async_=True
                    )
                    ...
                    for column_info in cursor.fetchall():
                        ...
                        if columns_section:
                            ...
                            column_info[1]
                            and column_info[1].strip() == "comment"
                        ):
        ...
        DataSourceInferSchemaApi._delete_old_attributes(ds)
        for i, (name, data_type, raw_type, description) in enumerate(
            zip_longest(final_names, types, raw_types, descriptions)
        ):
    ...
    def _delete_old_attributes(ds):
        ...
        old_privacy_attrs = AttributePrivacy.query.filter(
            AttributePrivacy.attribute_id.in_(
                old_attrs.with_entities(AttributePrivacy.attribute_id)
            )
        )
    ...
    def post(data_source_id):
        ...
        try:
            ...
            result = (
                {"status": "ERROR", "message": gettext("Invalid CSV encoding")},
                400,
            )
            ...
            result = (
                {
                    "status": "ERROR",
                    "message": gettext("Internal error, try later"),
                },
                400,
            )
        except Exception:
    ...
    def _get_csv_attributes(attrs, csv_reader, use_header, missing_values):
        for row in csv_reader:
            if use_header and len(attrs) == 0:
                ...
                if len(attrs) == 0:
                    attrs = DataSourceInferSchemaApi._get_default_header(row)
    ...
    def _infer_attr(attrs, i, value):
        ...
        if any(
            [
                (value[0] == "0" and len(value) > 1 and value[1] != "."),
                type(v) in [str, str],
            ]
        ):
            ...
            if not change_to_str:
                ...
                attrs[i].precision = min(
                    max(
                        (attrs[i].precision or 0),
    ...
    def _get_default_header(row):
        ...
        for i, attr in enumerate(row):
            ...
            attrs.append(
                Attribute(
                    name="attr{}".format(i), nullable=False, enumeration=False
                )
            )
        return attrs
    ...
    def _get_header(row):
        ...
        for attr in row:
            ...
            attrs.append(Attribute(name=attr, nullable=False, enumeration=False))
        return attrs
...
class DataSourcePrivacyApi(Resource):
    ...
    def get(data_source_id):
        ...
        attr_privacy = (
            DataSource.query.outerjoin(DataSource.attributes)
        ...
        if attr_privacy:
            return {
                "data": DataSourcePrivacyResponseSchema().dump(
...
class DataSourceSampleApi(Resource):
    ...
    def _get_sample(data_source_id: int):
        ...
        data_source = data_source.filter(DataSource.id == data_source_id).first()
        ...
        if limit > 1000:
            result, status_code = (
                dict(
                    status="ERROR",
                    message="The maximum number of records allowed is 1000",
                ),
                400,
            )
            ...
            if data_source.format == 'JDBC':
                ...
                if data_source.format == DataSourceFormat.CSV:
                    ...
                    csvfile = (
                        gzip.open(parsed.path, mode="rt")
                        if parsed.path.endswith(".gz")
                        else codecs.open(parsed.path, "rb", encoding=encoding)
                    )
                ...
                try:
                    ...
                    result, status_code = (
                        dict(
                            status="ERROR",
                            message=gettext(
                                "Data type are not correctly defined. Please, revise them."
                            ),
                        ),
                        400,
                    )
                ...
                return dict(
                    status="ERROR",
                    message="Unsupported protocol {}".format(parsed.scheme),
                ), 400
        else:
